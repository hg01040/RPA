...
def loginDB(userId, userPassword):
    ...
    conn = sqlite3.connect('education.db') ## 데이터베이스에서 fapi로 연결해주고 정상적으로 연결이 되면 cursor 생성
    cursor = conn.cursor()                 
    
    query = 'SELECT * FROM user WHERE userId = ? AND userPassword = ?'  ## query 객체 생성
    cursor.execute(query, (userId, userPassword))                       ## conn을 통해 받아온 데이터를 cursor에 들어감
    result = cursor.fetchone()                                          ## cursor에 들어간 데이터를 fetch를 이용해서 끄집어냄
    conn.close()
    ...
    if result:
        ...
        print("Login failed. Invalid username of password.")
        return False
